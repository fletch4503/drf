...
class pwp_exch_model:
    ...
    msg_cnt_list = [
        ...
        0,
        0,
    ]  # Обнуляем список счетчиков полученных сообщений в папке inbox
    ...
    def __init__(self):  # Подключаемся к Exchange-серверу и проверяем подключение
        try:
            self.credents_project = Credentials(
                username=exch_username, password=exch_userkey
            )
        ...
        self.my_acc_exch = Account(
            primary_smtp_address=exch_usersmtpaddr,
            config=self.conf_exchange,
            credentials=self.credents_project,
            autodiscover=False,
        )
        ...
        logger.info(
            "Текущая тайм-зона по datetime: %s ",
    ...
    def count_inbox_msg(self):
        ...
        self.msg_cnt_list[0] = (
            self.my_acc_exch.inbox.total_count
        )  # Всего сообщений в папке Входящие
        self.msg_cnt_list[1] = (
            self.my_acc_exch.inbox.unread_count
        )  # Непрочитанных сообщений в папке Входящие
        all_items = (
            self.my_acc_exch.inbox // inb_fold_supp
        )  # Всего сообщений в папке Поставщики
        self.msg_cnt_list[2] = all_items.total_count
        all_items = (
            self.my_acc_exch.inbox // inb_fold_sales
        )  # Всего сообщений в папке МТСТ
        self.msg_cnt_list[3] = all_items.total_count
        all_items = (
            self.my_acc_exch.inbox // inb_fold_other
        )  # Всего сообщений в папке Другие
        ...
        for attachment in item.attachments:
            if isinstance(attachment, FileAttachment):
                ...
                if isinstance(attachment.item, Message):
                    try:
                        print(
                            "create_attachement_list -> Сообщение:",
                            attachment.item.subject,
    ...
    @staticmethod
    def msg_move2folder(
        msg2move, folder, mark_unread
    ):  # Перемещаем сообщение msg2move в папку folder
        if mark_unread == 0:
